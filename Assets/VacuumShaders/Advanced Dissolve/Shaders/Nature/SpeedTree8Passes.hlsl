...
#ifndef SCENESELECTIONPASS
float4 dissolaveAlpha = AdvancedDissolveGetAlpha(input.interpolated.uv, input.interpolated.positionWS.xyz, input.interpolated.normalWS.xyz, input.interpolated.dissolveUV);
...
#ifndef SCENESELECTIONPASS
float4 dissolaveAlpha = AdvancedDissolveGetAlpha(input.uv, input.positionWS.xyz, input.normalWS.xyz, input.dissolveUV);
DoDissolveClip(dissolaveAlpha); 
#endif
