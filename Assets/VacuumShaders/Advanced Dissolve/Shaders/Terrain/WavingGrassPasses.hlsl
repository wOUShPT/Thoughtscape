...
GrassVertexOutput WavingGrassVert(GrassVertexInput v)
{
    ...
    ADVANCED_DISSOLVE_INIT_DATA(o.clipPos, o.uv.xy, o.uv.xy, o)
...
float4 dissolaveAlpha = AdvancedDissolveGetAlpha(input.uv.xy, input.posWSShininess.xyz, input.normal, input.dissolveUV);
...
half4 DepthOnlyFragment(VertexOutput IN) : SV_TARGET
{
    ...
    float4 dissolaveAlpha = AdvancedDissolveGetAlpha(IN.uv.xy, IN.positionWS.xyz, IN.normalWS, IN.dissolveUV);
    DoDissolveClip(dissolaveAlpha); 
