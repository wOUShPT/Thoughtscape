...
half4 SplatmapFragment(Varyings IN) : SV_TARGET
{
    ...
    float4 dissolveData = ComputeScreenPos(mul(GetWorldToHClipMatrix(), float4(IN.positionWS.xyz, 1)));  
#else

    //float4 dissolveData = VALUE_UVSET == 0 ? float4(IN.uvMainAndLM.xy, 0, 0) : float4(IN.uvMainAndLM.zw, 0, 0); ---------- terrain has no uv2

    float4 dissolveData = float4(IN.uvMainAndLM.xy, 0, 0);		
#endif	
 

float4 dissolaveAlpha = AdvancedDissolveGetAlpha(IN.uvMainAndLM.xy, IN.positionWS.xyz, IN.normal, dissolveData);
DoDissolveClip(dissolaveAlpha); 

float3 dissolveAlbedo = 0; 
float3 dissolveEmission = 0;
float dissolveBlend = DoDissolveAlbedoEmission(dissolaveAlpha, dissolveAlbedo, dissolveEmission, IN.uvMainAndLM.xy, 1);
    ...
    float3 emission = lerp(float3(0, 0, 0), dissolveEmission, dissolveBlend);
...
// #endif
    ...
    o.normalWS = normalWS;
    ADVANCED_DISSOLVE_INIT_DATA(clipPos, v.texcoord.xy, v.texcoord.xy, o)
...
half4 ShadowPassFragment(VaryingsLean IN) : SV_TARGET
{
    ...
    float4 dissolaveAlpha = AdvancedDissolveGetAlpha(IN.texcoord.xy, IN.positionWS, IN.normalWS, IN.dissolveUV);
    DoDissolveClip(dissolaveAlpha); 
...
// #endif	
    ...
    ADVANCED_DISSOLVE_INIT_DATA(o.clipPos, v.texcoord.xy, v.texcoord.xy, o)
